...
const fetchScans = async () => {
  ...
  if (fetchedScans) {
    ...
    console.log(
      '[PatientDetailsModal] <== fetchScans SUCCESS. Scans:',
      JSON.parse(JSON.stringify(fetchedScans)),
    )
  } else {
...
watch(
  ...
  (isShown) => {
    ...
    if (isShown) {
      console.log(
        '[PatientDetailsModal] Modal is now visible. Patient prop data:',
        JSON.parse(JSON.stringify(props.patient)),
      )
    }
...
const handleSavePatientScan = async (scanDataFromForm) => {
  console.log(
    '%c[PatientDetailsModal] ==> handleSavePatientScan called.',
    'color: orange; font-weight: bold;',
  )
  console.log(
    '[PatientDetailsModal] Data received from form (@save event):',
    JSON.parse(JSON.stringify(scanDataFromForm)),
  )

  if (!props.patient?.id) {
    console.error(
      '[PatientDetailsModal] SAVE FAILED: props.patient.id is missing.',
      JSON.parse(JSON.stringify(props.patient)),
    )
  ...
  if (!scanDataFromForm.scanDate || isNaN(new Date(scanDataFromForm.scanDate))) {
    console.error(
      `[PatientDetailsModal] SAVE FAILED: Invalid date value received: "${scanDataFromForm.scanDate}"`,
    )
  ...
  const payload = {
    ...
    doctorDose:
      typeof scanDataFromForm.doctorDose === 'number' ? scanDataFromForm.doctorDose : null,
  }
  delete payload.dose

  console.log(
    '%c[PatientDetailsModal] Final payload being sent to DB store:',
  ...
  try {
    ...
    if (success) {
      ...
      console.error(
        '[PatientDetailsModal] <== handleSavePatientScan FAILED. DB store error:',
        databaseStore.error,
      )
    ...
    console.error(
      '%c[PatientDetailsModal] <== handleSavePatientScan CRITICAL ERROR.',
...
const handleDeleteScan = async () => {
  ...
  console.log(
    '[PatientDetailsModal] Deleting scan:',
    JSON.parse(JSON.stringify(scanToDelete.value)),
  )
  // ... rest of function
}
...
function openEditScanModal(scan) {
  console.log(
    '[PatientDetailsModal] openEditScanModal called with scan:',
...
function openConfirmDeleteScanModal(scan) {
  console.log(
    '[PatientDetailsModal] openConfirmDeleteScanModal called with scan:',
    JSON.parse(JSON.stringify(scan)),
  )
              ...
              <li v-for="scan in scans" :key="scan.id" class="scan-item">
                <div class="scan-info">
                  ...
                  <span class="scan-dose"
                    >{{ currentLanguage === 'en' ? 'Dose:' : 'الجرعة:' }}
                    {{ scan.patientDose }} mSv</span
                  >
                </div>
