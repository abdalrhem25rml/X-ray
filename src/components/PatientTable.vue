      ...
      <table class="patient-table">
        ...
        <tbody>
          <tr v-for="patient in patients" :key="patient.id">
            <td>{{ patient.name }}</td>
            <td>{{ patient.age }}</td>
            <td>{{ currentLanguage === 'en' ? patient.gender : (patient.gender === 'male' ? 'ذكر' : 'أنثى') }}</td>
            <td>
              ...
              <button @click="$emit('recommend', patient.id)" class="action-button-sm recommend-button">
...
.patient-list-section h3 { color: #8d99ae; font-size: 1.6em; margin-bottom: 25px; font-weight: 600; }
.loading-message, .no-patients-message { text-align: center; font-size: 1.2em; color: #8d99ae; margin-top: 50px; display: flex; align-items: center; justify-content: center; gap: 10px; }
.patient-table-container { overflow-x: auto; width: 100%; border-radius: 8px; border: 1px solid #e0e0e0; box-shadow: 0 2px 8px rgba(0,0,0,0.05); background-color: white; }
.patient-table { width: 100%; border-collapse: collapse; }
.patient-table th, .patient-table td { padding: 15px; text-align: left; border-bottom: 1px solid #eee; vertical-align: middle; }
.patient-table th { background-color: #f7f7f7; color: #555; font-weight: 600; text-transform: uppercase; font-size: 0.9em; }
.patient-table tbody tr:hover { background-color: #f0f4f8; }
.action-button-sm { background: none; border: none; color: #8d99ae; padding: 8px 12px; border-radius: 6px; cursor: pointer; font-size: 1.2em; margin: 0 4px; transition: all 0.2s ease; }
.action-button-sm:hover { transform: translateY(-1px); }
