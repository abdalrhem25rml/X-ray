...
export const useDatabaseStore = defineStore('database', {
  ...
  actions: {
   ...
   // --- PATIENT DATA ACTIONS (Refactored for Standalone Patients) ---
    async createPatientProfile(patientData) {
      console.log(`[DB_STORE] createPatientProfile: Attempting to create new patient with data:`, patientData);
      this.loading = true;
      this.error = null;
      ...
      try {
        ...
        const dataToSave = {
          ...patientData,
          createdBy: userId, // Link patient to the creating doctor
          createdAt: serverTimestamp(),
          updatedAt: serverTimestamp(),
        };
        console.log('[DB_STORE] createPatientProfile: Final data being saved:', dataToSave);
    ...
    async updatePatientProfile(patientId, patientData) {
      console.log(`[DB_STORE] updatePatientProfile: Attempting to update patient ID: ${patientId} with data:`, patientData);
      this.loading = true;
      this.error = null;
      const essentials = this._getDBEssentials();
      ...
      try {
        ...
        await updateDoc(patientDocRef, { ...patientData, updatedAt: serverTimestamp() });
        console.log(`[DB_STORE] updatePatientProfile: Successfully updated patient ${patientId}.`);
        return true;
      } catch (err) {
        console.error(`[DB_STORE] updatePatientProfile: Error updating patient profile ${patientId}:`, err);
        this.error = err.message;
        return false;
      } finally {
        this.loading = false;
    ...
    async fetchPatientsForDoctor() {
      ...
      try {
        const { db, appId, userId } = essentials;
        const patientsCollectionRef = collection(db, 'artifacts', appId, 'patients');
        const q = query(patientsCollectionRef, where('createdBy', '==', userId), orderBy('createdAt', 'desc'));
    ...
    async createScan(scanData) {
      ...
      this.loading = true; this.error = null; const essentials = this._getDBEssentials(); if (!essentials) return null;
      try {
        const { db, appId, userId } = essentials;
        const scansCollectionRef = collection(db, 'artifacts', appId, 'scans');
        const newScanData = { ...scanData, creatorId: userId, isPersonalScan: scanData.patientId === userId, createdAt: serverTimestamp() };
        ...
        console.error('[DB_STORE] createScan: Error:', err); this.error = err.message; return null;
      } finally {
        this.loading = false;
      }
    },
    async updateScan(scanId, scanData) {
      ...
      this.loading = true; this.error = null; const essentials = this._getDBEssentials(); if (!essentials) return false;
      try {
        const { db, appId } = essentials;
        const scanDocRef = doc(db, 'artifacts', appId, 'scans', scanId);
        await updateDoc(scanDocRef, { ...scanData, updatedAt: serverTimestamp() });
        console.log(`[DB_STORE] updateScan: Successfully updated scan ${scanId}.`);
        return true;
      } catch (err) {
        console.error(`[DB_STORE] updateScan: Error updating scan ${scanId}:`, err); this.error = err.message; return false;
      } finally {
        this.loading = false;
      }
    },
    async deleteScan(scanId) {
      ...
      try {
        ...
        console.error(`[DB_STORE] deleteScan: Error deleting scan ${scanId}:`, err); this.error = err.message; return false;
      } finally {
        this.loading = false;
      }
    },
    async fetchScansForPatient(patientId) {
      ...
      try {
        ...
        const scans = []; querySnapshot.forEach((doc) => { scans.push({ id: doc.id, ...doc.data() }) });
        return scans;
      } catch (err) {
        console.error('[DB_STORE] fetchScansForPatient: Error:', err); this.error = err.message; return null;
      } finally {
        this.loading = false;
      }
    },
    async fetchDoctorCreatedScans() {
      ...
      try {
        ...
        console.error('[DB_STORE] fetchDoctorCreatedScans: Error:', err); this.error = err.message; return null;
      } finally {
        this.loading = false;
      }
    },
    ...
    async addPregnancyDeclaration(declarationData) {
      ...
      try {
        ...
        const newDeclaration = { ...declarationData, status: 'active', endDate: null, monitoringPeriodStartDate, createdAt: serverTimestamp() };
        ...
        console.error('[DB_STORE] addPregnancyDeclaration: Error:', err); this.error = err.message; return null;
      } finally {
        this.loading = false;
      }
    },
    async fetchPregnancyHistory() {
      ...
      try {
        ...
        const history = []; querySnapshot.forEach((doc) => { history.push({ id: doc.id, ...doc.data() }) });
        console.log('[DB_STORE] fetchPregnancyHistory: Returning history:', history);
        return history;
      } catch (err) {
        console.error('[DB_STORE] fetchPregnancyHistory: Error:', err); this.error = err.message; return null;
      } finally {
        this.loading = false;
      }
    },
    async updatePregnancyRecord(pregnancyId, updates) {
      ...
      try {
        ...
        console.error("[DB_STORE] updatePregnancyRecord: Error:", err); this.error = err.message; return false;
      } finally {
        this.loading = false;
      }
    },
